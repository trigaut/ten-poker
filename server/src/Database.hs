...
dbGetUserByEmail connString email = do
  maybeUserEntity <- runAction connString
...
cacheUser redisConfig email userE = runRedisAction redisConfig $ void $ setex
  (pack . show $ email)
  3600
  (pack . show $ userE)
...
dbUpdateUsersChips connString userChipCounts = runAction
  connString
  (updateWhere ((UserEntityUsername ==.) . fst <$> userChipCounts)
...
dbDepositChipsIntoPlay connString username chipsToAdd = runAction
  connString
  (updateWhere
...
dbWithdrawChipsFromPlay connString username chipsToAdd = runAction
  connString
  (updateWhere
    [UserEntityUsername ==. username]
    [ UserEntityAvailableChips +=. chipsToAdd
...
dbRefillAvailableChips connString refillThreshold = runAction
  connString
  (updateWhere [UserEntityAvailableChips <. refillThreshold]
